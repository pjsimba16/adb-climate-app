...
# S3 config: bucket where the home-page snapshots now live
S3_BUCKET = "adb-climate-data"  # your bucket name
S3_PREFIX = ""                  # no extra prefix; files are at bucket root

def get_data_source_label() -> str:
    """
    Label for the hero badge showing where data was actually read from last
    (S3 vs Hugging Face).
    """
    backend = st.session_state.get("_last_backend")
    if backend == "s3":
...
def _read_parquet_from_space(rel_path: str, columns: list[str]) -> Optional[pd.DataFrame]:
    """
    Generic helper to read a parquet file for the home page.

    New behaviour:
      1. Try S3 (adb-climate-data bucket) first.
      2. If that fails, fall back to the old Hugging Face HTTP method.

    `columns` is kept for API compatibility; if provided, it's passed to
    pandas.read_parquet to select only required columns.
    """
    key = rel_path.lstrip("/")
    if S3_PREFIX:
    ...
    try:
        ...
        # record backend for the hero badge
        st.session_state["_last_backend"] = "s3"
        return df
    except Exception as e:
        _note_err(
            f"S3 read failed for {rel_path} (bucket={S3_BUCKET}, key={key}): {e}"
        )
    ...
    try:
        ...
        st.session_state["_last_backend"] = "hf"
    ...
    try:
        ...
        if columns:
            kwargs["columns"] = columns
        return pd.read_parquet(io.BytesIO(resp.content), **kwargs)
    except Exception as e:  # pragma: no cover
    ...
    st.markdown(
        ...
        <div class="panel-left">
          ...
          <div class="badgerow">
            ...
            <span class="badge">Data source: <b>{data_source_label}</b></span>
          </div>
        </div>
        """,
        unsafe_allow_html=True,

...
# --- Remote data backends (S3 first, HF fallback) ------------------------------
HF_SPACE_BASE = "https://huggingface.co/spaces/pjsimba16/adb-climate-data/resolve/main"
S3_BUCKET = "adb-climate-data"  # your S3 bucket: s3://adb-climate-data/
...
def get_data_source_label() -> str:
    """
    Label used in the 'Data source' text on this page, based on the last
    successful remote read (S3 or HF).
    """
...
def _read_parquet_from_space(rel_path: str,
    ...
    """
    Read a parquet file from remote storage.

    New behaviour:
      1. Try S3 (adb-climate-data bucket) first.
      2. If S3 fails, fall back to the old Hugging Face HTTP method.
    ...
    # --- 2) Fallback to HF HTTP if needed ---
    if data is None:
    ...
    # --- Parse parquet, with column subset & fallback ---
    try:
        kwargs = {}
        if columns is not None:
            kwargs["columns"] = columns
        df = pd.read_parquet(io.BytesIO(data), **kwargs)
        st.session_state["data_source_backend"] = backend
        return df
    except Exception:
        try:
...
with top:
    ...
    with c2:
        ...
        st.markdown(
            f"<div class='helper-text'>Build and compare custom climate charts across countries and ADM1s. "
            f"Choose indicators, chart types (including anomalies), frequency, date windows, and facets; "
            f"apply smoothing, normalization, and climatology overlays; then export the figure and data. "
            f"Data source: {data_source_label}</div>",
            unsafe_allow_html=True,
        )

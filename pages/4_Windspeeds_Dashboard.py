...
except Exception:  # boto3 not available, we'll just skip S3 and use HF
    boto3 = None
...
# S3 config: your bucket where all the indicator data now lives
S3_BUCKET = "adb-climate-data"  # s3://adb-climate-data/ (no extra prefix)
...
def get_data_source_label() -> str:
    """
    Human-readable label for where the indicator data was actually read from
    (S3 vs Hugging Face). Used for the 'Data source:' badge under the map.
    """
    backend = st.session_state.get("data_source_backend", "S3")
    if backend == "S3":
        return "adb-climate-data (S3 bucket)"
    if backend == "HF":
        return "adb-climate-data (Hugging Face Space)"
    return str(backend)

# --- GeoJSON backend helpers (for map badge) ---

def _set_geojson_source_backend(source: str) -> None:
    try:
        st.session_state["geojson_source_backend"] = source
    except Exception:
        pass

def get_geojson_source_label() -> str:
    """
    Human-readable label for where the ADM1 GeoJSON came from.
    """
    backend = st.session_state.get("geojson_source_backend", "S3")
    if backend == "S3":
        return "ADM1_geodata (S3 bucket)"
    if backend == "HF":
        return "ADM1_geodata (Hugging Face Space)"
    return str(backend)
...
def _read_parquet_from_space(rel_path: str):
    """
    Generic helper to read a parquet file for this dashboard.

    NEW behaviour:
      1. Try S3 (adb-climate-data bucket) first.
      2. If that fails, fall back to the older Hugging Face HTTP method.

    This keeps the rest of the file unchanged: every existing call to
    _read_parquet_from_space(...) will automatically use S3 by default.
    """
    key = rel_path.lstrip("/")
    ...
    try:
        ...
        # record backend for the map badge
        st.session_state["data_source_backend"] = "S3"
        return df
    except Exception as e:
        _note_err(f"S3 read failed for {rel_path} (bucket={S3_BUCKET}, key={key}): {e}")

    # --- 2) Fallback to HF HTTP (old behaviour) ---
    ...
    try:
        ...
        st.session_state["data_source_backend"] = "HF"
        return df
    except Exception as e:  # pragma: no cover
...
@st.cache_data(ttl=24 * 3600, show_spinner=False)
def _windspeeds_isos_from_availability():
    snap = _load_availability_snapshot()
    if snap is None or snap.empty:
        return []
    # after _canonical_type, Windspeeds rows will have indicator_type == "Wind speeds"
    mask = snap["indicator_type"].astype(str).str.lower() == "wind speeds"
    return sorted(
        snap.loc[mask, "iso3"].astype(str).str.upper().unique().tolist()
    )
...
def list_available_isos():
    """List ISO3 codes with Windspeeds data, based on HF availability_snapshot."""
    try:
        isos = _windspeeds_isos_from_availability()
    except Exception:
        isos = []
    return list(isos or [])
...
def load_country_adm1_geojson(iso3: str):
    """
    Load ADM1 GeoJSON, preferring S3 (adb-climate-data) and falling back to HF.

    After loading, apply composite layout (USA/FRA/ESP/PRT/DNK/NOR/CHN get their
    territories pulled in as insets) and return:
      - geojson dict for Plotly
      - bounds (minx, miny, maxx, maxy)
      - name_col used for labels
      - full composed GeoDataFrame
    """
    iso3u = (iso3 or "").upper()
    gdf = None
    ...
    try:
        ...
        s3 = boto3.client("s3")
        key = f"ADM1_geodata/{iso3u}.geojson"
        obj = s3.get_object(Bucket=S3_BUCKET, Key=key)
        ...
        # If CRS is specified in the GeoJSON, use it
        try:
            crs_name = (
                data.get("crs", {})
                .get("properties", {})
        ...
        _set_geojson_source_backend("S3")

    except Exception as e:
        last_err = e
        gdf = None

    # 2. Fall back to HF if S3 is unavailable or file missing
    if gdf is None:
        if hf_hub_download is None:
            raise FileNotFoundError(
                "Failed to load ADM1 GeoJSON from S3 and huggingface_hub "
        ...
        for repo_type in ("space", "dataset"):
            try:
                ...
                _set_geojson_source_backend("HF")
                break
            except Exception as e:
                last_err = e
                continue
        ...
        if gdf is None:
            raise last_err or FileNotFoundError(
                f"ADM1 GeoJSON not found in S3 or HF for {iso3u}"
            )

    # 3. Geometry clean-up + composite layout (same as before)
    try:
...
@st.cache_data(ttl=24 * 3600, show_spinner=False)
def _load_windspeeds_adm1_snapshot():
    """
    Latest ADM1 WSPA snapshot for all countries/frequencies (for map).

    Tries a couple of plausible snapshot filenames in the HF space.
    """
    candidates = [
        "indicator_snapshots/windspeeds_adm1_latest.parquet",
        "indicator_snapshots/windspeed_adm1_latest.parquet",
    ]

    df = None
    for rel_path in candidates:
        df = _read_parquet_from_space(rel_path)
        if isinstance(df, pd.DataFrame) and not df.empty:
            break

    if df is None or df.empty:
        _note_err("No usable windspeeds ADM1 snapshot found in HF space.")
        return pd.DataFrame()
    ...
    if not value_col:
        # Fall back to first column that looks like a WSPA/wind column
        for c in df.columns:
            cl = str(c).lower()
            if "wspa" in cl or "windspeed" in cl or "wind_speed" in cl or "wind" in cl:
                value_col = c
                break

    if not iso_col or not adm1_col or not date_col or not value_col:
        _note_err("windspeeds ADM1 snapshot missing required columns.")
        return pd.DataFrame()
...
def load_latest_adm1_for_map(iso3: str, freq: str):
    """
    Return the latest available ADM1 WSPA value per region for the country,
    using the precomputed ADM1 snapshot stored in the HF Space.

    Returns a DataFrame with columns ['ADM1', 'value', 'Date'].
    """
    snap = _load_windspeeds_adm1_snapshot()
    ...
    # Try to use the requested frequency; if none, fall back to any
...
with lc:
    ...
    if iso3 and iso3 != "—":
        ...
        if df_latest.empty:
            ...
            geojson_source_label = get_geojson_source_label()
            ...
            st.markdown(
                ...
                <span style="font-size:12px;padding:4px 8px;border-radius:999px;border:1px solid #cbd5e1;color:#334155;">
                    GeoJSON source: {geojson_source_label}
                </span>
                </div>
                """,
                unsafe_allow_html=True,
...
with rc:
    ...
    if base_wspa.empty:
        ...
        with k1: render_kpi("Latest Avg Wind speed (WSPA)", f"{latest:.2f} m/s", None, show_symbol=False)
        ...
        with k4: render_kpi("Mean / σ in range", (f"{mean_v:.2f} m/s • {std_v:.2f}" if np.isfinite(mean_v) and np.isfinite(std_v) else "—"), None, show_symbol=False)
...
def set_legend_top(fig):
    # Actually place legend at the bottom for all windspeeds charts
    fig.update_layout(
        legend=dict(
            orientation="h",
            x=0.0,
        ...
        # Only touch bottom margin so we don't clobber per-chart top margins
        margin=dict(b=80),
    )
...
def _render_ws_chart(chart_key, story_title_md, plot_title, avg_code, var_code=None, *, ylab="m/s", show_form=False):
    ...
    if show_form:
        ...
        # Charts without a side form (if we ever call _render_ws_chart that way)
    ...
    with chart_col:
        ...
        if show_form:
            # For WSPA, hide-country is driven by the side form checkbox
            if adm1_sel:
            ...
            # If we ever call _render_ws_chart without a form, manage it inline
            if adm1_sel:
...
def _pct_block(title: str, code: str, chart_key: str, story: str):
    ...
    # --- right: ADM1 selector + hide-country + slider + chart ---
    with chart_col:
        # Per-chart ADM1 options (from the global _adm1_all_pct we built above)
        try:
        ...
        # bounds for sliders; prefer Country baseline if present
        if "Country (all)" in sdict:
            s_base = sdict["Country (all)"].dropna(subset=["date"]).sort_values("date")
        else:
            s_base = next(iter(sdict.values())).dropna(subset=["date"]).sort_values("date")

        if s_base.empty:
            st.info("No data for percentiles.")
            return

        dmin = s_base["date"].min().date()
        dmax = s_base["date"].max().date()
        ...
        set_legend_top(fig)  # already patched to move legend to bottom
        if freq == "Monthly":
